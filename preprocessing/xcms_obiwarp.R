...
##############################################################################
#
# Example of how to read these formats in R and to get 3 tables with data.
#
# x <- readRDS(file = "xcms_camera_sumexp.rds")
...
log("Running adjustRtime (Obiwarp)")
x <- settings$xcms_parameters$alignment
pdp <- xcms::ObiwarpParam(
  binSize        = x$binSize,
  response       = x$response,
  distFunc       = x$distFunc,
  gapInit        = x$gapInit,
  gapExtend      = x$gapExtend,
  factorDiag     = x$factorDiag,
  factorGap      = x$factorGap,
  localAlignment = x$localAlignment,
  initPenalty    = x$initPenalty,
  centerSample   = x$centerSample
)
